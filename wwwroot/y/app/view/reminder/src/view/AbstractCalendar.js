...
Ext.define('iBOSe.view.reminder.src.view.AbstractCalendar', {
            ...
            for (d = 0; d < this.dayCount; d++) {
                if (evtsInView.getCount() > 0) {
                    var evts = evtsInView.filterBy(function(rec) {
                        var startDt = Ext.Date.clearTime(new Date(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name]), true),
                            startsOnDate = dt.getTime() == startDt.getTime(),
                            spansFromPrevView = (w == 0 && d == 0 && (dt > new Date(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name])));
    ...
    prepareEventGrid: function(evts, w, d) {
        ...
        evts.each(function(evt) {
            var M = iBOSe.view.reminder.src.data.EventMappings.getFields()[0],
    ...
    onEventDrop: function(rec, dt) {
        if (iBOSe.view.reminder.src.util.Date.compare(new Date(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name]), dt) === 0) {
            // no changes
            return;
        }
        var sdt = new Date(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name]);
        var diff = dt.getTime() - sdt.getTime();
        rec.set(new Date(iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name), dt);
        rec.set(new Date(iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EndDate.name), iBOSe.view.reminder.src.util.Date.add(new Date(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EndDate.name]), {millis: diff}));
    ...
    onCalendarEndDrag: function(start, end, onComplete) {
        if (start && end) {
            ...
            var o = {};
            o[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].StartDate.name] = start;
            o[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EndDate.name] = end;
    ...
    onUpdate: function(ds, rec, operation) {
        ...
        if (operation == Ext.data.Record.COMMIT) {
            ...
            if (this.enableFx && this.enableUpdateFx) {
                this.doUpdateFx(this.getEventEls(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EventId.name]), {
                    scope: this
                });
            }
        }
    ...
    onAdd: function(ds, records, index) {
        ...
        if (this.enableFx && this.enableAddFx) {
            this.doAddFx(this.getEventEls(rec.data[iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EventId.name]), {
                scope: this
            });
        }
    },
    ...
    onRemove: function(ds, recs) {
        var name = iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EventId.name,
            i, len, rec, els;
    ...
    getEventRecord: function(id) {
        var idx = this.store.find(iBOSe.view.reminder.src.data.EventMappings.getFields()[0].EventId.name, id);
